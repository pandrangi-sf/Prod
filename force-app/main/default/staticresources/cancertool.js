...
//*** NC][ mortality 1998:02,    filipino  Jan052010;
...
//*** SE][18 incidence 1998:02 - hawaiian  Jan052010;
...
//*** NC][ mortality 1998:02,    hawaiian  Jan052010;
...
//*** SE][18 incidence 1998:02 - other pacific islander  Jan052010;
...
//*** NC][ mortality 1998:02,    other pacific islander  Jan052010;
...
//*** SE][18 incidence 1998:02 - other asian  Jan052010;
...
//*** NC][ mortality 1998:02,    other asian Jan052010;
...
function getRisk(hyperplasia, everHadBiopsy){
	switch (hyperplasia){
 ...
 function CurrentAgeIndicator(currentAge){
    rval = 0;
    if (currentAge < 50)
        rval = 0;
...
$(function(){
	$("form").submit(function(){
		CurrentAge           = parseInt($('.age  ').val());
		ProjectionAge        = parseInt($('.age  ').val())+5;
		AgeIndicator         = CurrentAgeIndicator(parseInt($('.age').val()));
		NumberOfBiopsy       = parseInt($('.bio  ').val());
		MenarcheAge          = parseInt($('.mage ').val());
		FirstLiveBirthAge    = parseInt($('.fbage').val());
		EverHadBiopsy        = parseInt($('.fdrel').val());
		FirstDegRelatives    = parseInt($('.hadb ').val());
		ihyp                 = parseInt($('.ihyp ').val());
		rhyp                 = getRisk(parseInt($('.rhyp ').val()), 0);
		irace                = parseInt($('.race ').val());
		
		a=CalculateRisk(1,CurrentAge, ProjectionAge, AgeIndicator, NumberOfBiopsy, MenarcheAge, FirstLiveBirthAge, EverHadBiopsy, FirstDegRelatives, ihyp, rhyp, irace);
		b=CalculateRisk(2,CurrentAge, ProjectionAge, AgeIndicator, NumberOfBiopsy, MenarcheAge, FirstLiveBirthAge, EverHadBiopsy, FirstDegRelatives, ihyp, rhyp, irace);
		alert(a);
		alert(b);
		return false;
	});
...
function checkRisk(CurrentAge       , 
	...
	vala1 = parseFloat(a * 100).toFixed(1);
	vala2 = parseFloat(b * 100).toFixed(1);
...
function checkRisk2(CurrentAge       , 
				   ProjectionAge    , 
				   AgeIndicator     , 
				   NumberOfBiopsy   , 
				   MenarcheAge      , 
	...
	valb1 = parseFloat(a * 100).toFixed(1);
	valb2 = parseFloat(b * 100).toFixed(1);
...
//CalculateRisk(0,35,40,0,1,0,2,0,1,0,1.82,1);
function CalculateRisk( riskindex                 //1
					   ,CurrentAge                //2
					   ,ProjectionAge             //3
					   ,AgeIndicator              //4
					   ,NumberOfBiopsy            //5
	...
	abss = abs[i - 1] * 1000.0;
    if (abss - parseInt(abss) >= .5){
        //abss = d_int(abss) + 1.0 ;
        abss = parseInt(abss) + 1.0;
    }
    else
    {
        abss = parseInt(abss);
